	...
	for (int i = 0; i < P.elements.size(); i++) {
		...
		for (int j = 0; j < P.elements.size(); j++) {
			...
			if (px.p == py.p) {
				...
				for (int r = 0; r < scopex.size(); r++)
					for (int t = 0; t < scopey.size(); t++) {
						...
						if (scopex[r].tid == scopey[t].tid && scopex[r].m == scopey[t].m &&
							scopex[r].s.x <= scopey[t].s.x &&
							scopex[r].s.p >= scopey[t].s.p) {
							eleIn.scopes.push_back(Scope(scopex[r].tid, scopex[r].m,
														 scopex[r].s.x, scopey[t].s));
						}
					}
				if (eleIn.check())
				...
				{
					printf("Given prefix xx with new node yy, the pattern is not frequent.");
				}
			}
			// Out-scope Test
			if (py.p != -1) {
